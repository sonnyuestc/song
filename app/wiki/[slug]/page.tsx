...
export default async function WikiArticle({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const entry=entries.find(e=>e.slug===slug);if(!entry)notFound();const report=reports.find(r=>r.slug===slug);return <WikiShell><a className={styles.back} href={sitePath("/wiki/")}>← 返回 Wiki 目录</a><article className={styles.article}><div className={styles.eyebrow}>{slug==="pixelpilot"?"OPEN SOURCE / 开源社区":"FIELD REPORT / 实测案例"}</div><Markdown text={entry.body}/>{report&&<ReportViewer report={report}/>}</article></WikiShell>;}
